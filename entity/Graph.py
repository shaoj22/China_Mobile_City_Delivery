...
Description:
-----------
input the inputs file path and create Graph class for algorithm running.
-----------
Author: 626
...
import sys
sys.path.append('..')
from data import read_data as DataTools
import numpy as np
...
class Graph():
    def __init__(self, limit_nodeNum=None, limit_vehicleTypeNum=None, input_path=None):
        """
        init of the Graph
        
        Args:
            input_path (str): inputs file path.
        
        """
        data = DataTools.Data(input_path)
        self.data = data
        self._limit_nodeNum = limit_nodeNum
        self._limit_vehicleTypeNum = limit_vehicleTypeNum
        self.new_order_list, self.order_pair_list = self._preprocess(data)
        # evaluate paraments
        self.punish_for_overload = 1e10
        self.punish_for_overtime = 1e10
    ...
    def _preprocess(self, data):
        ...
        Args:
            data (object): data class of Data.
        ...
        if self._limit_nodeNum is None:
            order_list, order_pair_list = self._order_batching(data.order_list, vehicle_type_list)
        else:
            order_list, order_pair_list = self._order_batching(data.order_list, vehicle_type_list)[:self._limit_nodeNum]
        ...
        return order_list, order_pair_list
        ...
        for ri in range(1, len(order_list)):
            if order_list[ri].place == check_order.place and \
               ...
               order_list[ri].volumn + check_order.volumn <= volumn_capacity:
                ...
                # input
                if ri == 1:
                    new_old_order_pair.append(order_list[0].spuId)
                new_old_order_pair.append(order_list[ri].spuId)
            else:
                # add to list if not satisfy 
                new_order_list.append(check_order)
                check_order = order_list[ri]
                new_old_order_pair_list.append(new_old_order_pair)
                new_old_order_pair = []
                new_old_order_pair.append(order_list[ri].spuId)
        new_order_list.append(check_order)
        new_old_order_pair_list.append(new_old_order_pair)
        return new_order_list, new_old_order_pair_list
    ...
    def cal_objective(self, routes):
        ...
        Args:
            routes (list[[]]): solution of the instance.
        Return:
            obj (float): cost of the solution.
        """
        route_obj_list = []
        for route in routes:
    ...
    def visualize_route(self, coordinates, paths, save_path):
...
if __name__ == "__main__":
    input_path = "D:\\Desktop\\python_code\\China_Mobile_City_Delivery\\inputs\\myInstance3"
    graph = Graph(input_path=input_path)
